...
export default {
    ...
    methods: {
        ...
        handleTag(tagPath) {
            ...
            this.handleCacheView(defaultComponent.name)
        },
        //移除Tag
        removeTag(targetTag) {
        ...
        refreshSelectedTag(selectedTag) {
            if (selectedTag.linkTo === this.$route.path) {
                this.reload()
            }
            this.removeOneCacheView(selectedTag.componentName)
            this.$nextTick(() => {
                this.handleCacheView(selectedTag.componentName)
                this.$router.replace({ path: selectedTag.linkTo }).catch(err => err)
            })
        },
        ...
        //添加cachedViews
        handleCacheView(componentName) {
            const cachedViews = this.$store.state.iota[this.containerId || 'console'].cachedViews.map(v => v);
            if (componentName &&
                componentName != null &&
                cachedViews.indexOf(componentName) === -1) {
                cachedViews.push(componentName)
            }
        ...
        openMenu(tag, e) {
            ...
            const offsetDefault = this.collapsed ? 100 : 220
            const offsetLeft = this.$el.getBoundingClientRect().left // container margin left
            const offsetWidth = this.$el.offsetWidth // container width
            const maxLeft = offsetWidth + menuMinWidth // left boundary
            const left = e.clientX - offsetLeft + offsetDefault // margin right  会动态更改
            if (left > maxLeft) {
                this.left = maxLeft
            } else {
                this.left = left
